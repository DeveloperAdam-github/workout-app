...
<template>
  ...
  <router-link to="/workout-view"
    class="w-4/5 h-52 rounded-xl carousel-item mr-4 bg-cover bg-center flex flex-col text-white shadow-xl" :style="{
      'background-image': props.workout.tags.includes('upper', 'bench')
        ...
        : props.workout.tags.includes('lower', 'squats')
          ...
          : props.workout.tags.includes('full body')
            ...
            : '',
    }" @click="setChosenWorkout(props.workout)" v-if="props.workout">
    <div class="p-4 flex flex-col">
      <h1 class="text-lg">{{ props.workout.workout_name }}</h1>
      <div class="">
        <div class="p-2 w-max bg-gray-100 flex items-center my-2 text-xs rounded-xl text-black">
          <p>
            <i class="fa-solid fa-fire mr-2"></i>{{ props.workout?.calories }}Kcal
          </p>
        </div>
        <div class="p-2 w-max bg-gray-100 flex items-center my-2 text-xs rounded-xl text-black">
          <p>
